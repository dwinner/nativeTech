...
; function compare 2 strings ------------------------------------------------------------
compare2:
   mov   rcx, rdx   ; load string length
   cld
   repe  cmpsb      ; compare rsi/rdi bytes
   je    equal2
   mov   rax, strlen2
   sub   rax, rcx   ; compute position
   ret
...
cscan:
   mov   rcx, rdx
   lodsb            ; load a byte by the current rsi-address to rax
   cld              ; clear DF just in case
   repne scasb      ; compare al'byte with rdi'current content, repeat until not equal
   jne   char_notfound
   mov   rax, strlen3
   sub   rax, rcx   ; compute position
   ret
