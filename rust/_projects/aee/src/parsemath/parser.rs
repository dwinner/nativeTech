...
impl<'a> Parser<'a>
{
   // Retrieve the next token from arithmetic expression and set it to
   // current_token field in Parser struct
   ...
   // Construct AST node for numbers, taking into account negative prefixes
   // while handling parenthesis
...
impl fmt::Display for ParseError
{
   fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result
   {
      match &self
      {
         ParseError::UnableToParse(err) => write!(f, "Error in evaluating {}", err),
         ParseError::InvalidOperator(err) => write!(f, "Error in evaluating {}", err),
      }
   }
}
...
{
   fn from(_eval_err: Box<dyn std::error::Error>) -> Self
   {
