...
// If ContentType is TemplateVariable, the contents of the line are parsed and
// stored in the ExpressionData struct.alloc If template file contains this
// line: <p> Hello {{name}} ,welcome </p> then Head = "Hello" , variable =
// "name" and tail = ",welcome"
#[derive(PartialEq, Debug)]
...
pub fn get_index_for_symbol(input: &str, symbol: char) -> (bool, usize)
{
   ...
   let mut index = 0;
   while let Some((char_idx, fnd_char)) = characters.next()
   {
      if fnd_char == symbol
      {
         does_exist = true;
...
// Reads one line of template file and returns type of Content. The types of
// content are defined in ContentType enum If contentType is TemplateVariable,
// it parses the line further to separate out the head, variable and tail
// components. If ContentType is Literal, it returns the read input without any
// modifications.
pub fn get_content_type(input_line: &str) -> ContentType
{
   ...
   // ForTag expressions begin with  keywords 'for' and 'in' enclosed within {%
   // and %} ForTag expressions end with keyword 'endfor' enclosed within {%
   // and %}
   ...
   // 3) an optional tail
   // e.g. the expression <p> Hello {{name}} ,welcome </p> is parsed as follows:
...
pub fn generate_html_template_var(content: ExpressionData, context: HashMap<String, String>) -> String
{
   ...
   if let Some(head_str) = content.head
   {
      html.push_str(&head_str);
   }
   ...
   if let Some(tail_str) = content.tail
   {
      html.push_str(&tail_str);
   }
