...
impl fmt::Display for Elapsed
{
   fn fmt(&self, out: &mut fmt::Formatter) -> Result<(), fmt::Error>
   {
      ...
      {
         (0, nanos) if nanos < 1_000 => write!(out, "{} ns", nanos),
         (0, nanos) if nanos < 1_000_000 => write!(out, "{} µs", nanos / 1_000),
         (0, nanos) => write!(out, "{} ms", nanos / 1_000_000),
         (secs, nanos) if secs < 10 => write!(out, "{}.{:02} s", secs, nanos / 10_000_000),
         (secs, _) => write!(out, "{} s", secs),
      }
   }
}
...
fn resize_image(size: u32, src_folder: &mut PathBuf) -> Result<(), ImagixError>
{
   ...
   let new_file_name = src_folder
      ...
      .map(|file_name| format!("{}.png", file_name));

   // Construct path to destination folder i.e. create /tmp under source folder
   // if not exists
   ...
   // Open source image file, scale it to desired size and write output to
   // destination-folder/destination-file
...
pub fn get_image_files(src_folder: PathBuf) -> Result<Vec<PathBuf>, ImagixError>
{
   let entries = fs::read_dir(src_folder)
      ...
      .into_iter()
      .filter(|path_buf| {
         path_buf.extension() == Some("JPG".as_ref())
            || path_buf.extension() == Some("jpg".as_ref())
            || path_buf.extension() == Some("PNG".as_ref())
            || path_buf.extension() == Some("png".as_ref())
      })
      .collect();
   Ok(entries)
