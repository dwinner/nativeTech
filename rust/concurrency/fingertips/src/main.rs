...
//! different ways.
//!
//! * `run_single_threaded` simply does everything in one thread, in the most
//!   straightforward possible way.
//!
//! * Then, we break the work into a five-stage pipeline so that we can run it
//!   on multiple CPUs. `run_pipeline` puts the five stages together.
//!
...
) -> (mpsc::Receiver<InMemoryIndex>, thread::JoinHandle<()>)
{
   ...
   let handle = thread::spawn(move || {
      ...
      {
         accumulated_index.merge(file_idx);
         if accumulated_index.is_large()
         {
...
fn main()
{
...

   {
      ...
      arg_parser.set_description("Make an inverted index for searching documents.");
      arg_parser.refer(&mut single_threaded).add_option(
         &["-1", "--single-threaded"],
         StoreTrue,
         "Do all the work on a single thread.",
      );
      arg_parser.refer(&mut filenames).add_argument(
         "filenames",
         Collect,
         "\
                    Names of files/directories to index. For directories, all .txt files immediately under the \
          directory are indexed.",
      );
