...
{
   pub fn new(mut a_file: BufWriter<File>) -> io::Result<IndexFileWriter>
   {
      const HEADER_SIZE: u64 = 8;
      a_file.write_u64::<LittleEndian>(0)?;
      Ok(IndexFileWriter {
         offset: HEADER_SIZE,
         writer: a_file,
         contents_buf: vec![],
   ...
   pub fn write_contents_entry(&mut self, term: String, df: u32, offset: u64, bytes_num: u64)
   {
      self.contents_buf.write_u64::<LittleEndian>(offset).unwrap();
      self.contents_buf.write_u64::<LittleEndian>(bytes_num).unwrap();
   ...
   pub fn finish(mut self) -> io::Result<()>
   {
      ...
      println!(
         "{contents_start} bytes main, {} bytes total",
...
{
   let (filename, file) = tmp_dir.create()?;
   let mut writer = IndexFileWriter::new(file)?;

   // The merge algorithm requires the entries within each file to be sorted by
   // term. Sort before writing anything.
