...
pub struct Entry
{
   ...
   /// Offset of the index data for this term from the beginning of the file, in
   /// bytes.
   #[expect(dead_code)]
   pub offset: u64,
...
impl IndexFileReader
{
   ...
   fn read_entry(a_file: &mut BufReader<File>) -> io::Result<Option<Entry>>
   {
      // If the first read here fails with `UnexpectedEof`,
      // that's considered a success, with no entry read.
      let offset = match a_file.read_u64::<LittleEndian>()
      {
         Ok(value) => value,
         Err(err) =>
         {
      ...
      let num_of_bytes = a_file.read_u64::<LittleEndian>()?;
      let df = a_file.read_u32::<LittleEndian>()?;
      let term_len = a_file.read_u32::<LittleEndian>()? as usize;
      let mut bytes = vec![0; term_len];
      a_file.read_exact(&mut bytes)?;
   ...
   pub fn is_at(&self, term: &str) -> bool
   {
      match self.next
      {
         Some(ref entry) => entry.term == term,
         None => false,
      }
   }
   ...
   pub fn move_entry_to(&mut self, out: &mut IndexFileWriter) -> io::Result<()>
   {
      ...
      {
         let entry = self.next.as_ref().expect("no entry to move");
         if entry.byte_num > usize::MAX as u64
         {
            // This can only happen on 32-bit platforms.
            return Err(io::Error::other("computer not big enough to hold index entry"));
         }
         
         let mut buf = vec![0; entry.byte_num as usize];
