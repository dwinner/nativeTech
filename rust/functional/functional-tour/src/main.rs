...
fn main()
{
   let arr = [1, 2, 3, 4];
   println!("{:?}", arr);
   let vec: Vec<_> = arr.iter().map(|val| val.to_string()).collect();
   println!("{:?}", vec);

   let linked_list: LinkedList<i32> = vec.iter().flat_map(|val| val.parse::<i32>()).collect();
   println!("{:?}", linked_list);

   let arr = ["duck", "1", "2", "goose", "3", "4"];
   let (successes, failures): (Vec<_>, Vec<_>) = arr.iter().map(|val| val.parse::<i32>()).partition(Result::is_ok);
