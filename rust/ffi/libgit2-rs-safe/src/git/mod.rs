...
{
   unsafe {
      if let Err(err) = check(raw::git_libgit2_shutdown())
      {
         eprintln!("shutting down libgit2 failed: {}", err);
         std::process::abort();
      }
   }
}
...
impl Repository
{
   pub fn find_commit(&'_ self, oid: &Oid) -> Result<Commit>
   {
