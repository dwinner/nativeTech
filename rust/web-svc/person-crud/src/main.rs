...
fn get_page_persons(query: web::Query<Filter>, state: web::Data<Mutex<AppState>>) -> impl Responder
{
...
fn delete_persons(query: web::Query<ToDelete>, state: web::Data<Mutex<AppState>>) -> impl Responder
{
...
fn get_page_edit_person(state: web::Data<Mutex<AppState>>, path: web::Path<(String,)>) -> impl Responder
{
   let id = &path.0;
   let db_conn = &state.lock().unwrap().db;
   let mut context = tera::Context::new();
