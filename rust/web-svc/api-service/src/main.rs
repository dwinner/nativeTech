...
fn init_tracing()
{
   ...
   let rust_log =
      std::env::var(EnvFilter::DEFAULT_ENV).unwrap_or_else(|_| "sqlx=info,tower_http=debug,info".to_string());
...
async fn init_db_pool() -> Result<sqlx::Pool<sqlx::Sqlite>, sqlx::Error>
{
   ...
   sqlx::migrate!().run(&db_pool).await.expect("database migration failed");
...
{
   init_tracing();
   let db_pool = init_db_pool().await.expect("couldn't initialize DB pool");
