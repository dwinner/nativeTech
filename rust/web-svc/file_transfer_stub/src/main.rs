...
use actix_web::{App, HttpRequest, HttpResponse, HttpServer, web, web::Path};
use std::io::Write;
...
async fn delete_file(info: Path<(String,)>) -> HttpResponse
{
   let filename = &info.0;
...
async fn download_file(info: Path<(String,)>) -> HttpResponse
{
   let filename = &info.0;
...
async fn upload_specified_file(info: Path<(String,)>) -> HttpResponse
{
   let filename = &info.0;
...
async fn upload_new_file(info: Path<(String,)>) -> HttpResponse
{
   let filename = &info.0;
...
async fn invalid_resource(req: HttpRequest) -> HttpResponse
{
