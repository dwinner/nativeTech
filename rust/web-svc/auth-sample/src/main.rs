...
) -> Result<Vec<DbPrivilege>, String>
{
   ...
   if let Some(user) = db_conn.get_user_by_username(auth.user_id())
   {
      if auth.password().is_some() && &user.password == auth.password().unwrap()
      {
         ...
         {
            Err(format!("Insufficient privileges for user \"{}\".", user.username))
         }
      }
      else
      {
...
fn get_page_persons(query: web::Query<Filter>, auth: BasicAuth, state: web::Data<Mutex<AppState>>) -> HttpResponse
{
...
fn delete_persons(query: web::Query<ToDelete>, auth: BasicAuth, state: web::Data<Mutex<AppState>>) -> HttpResponse
{
...
fn get_page_edit_person(path: web::Path<(String,)>, auth: BasicAuth, state: web::Data<Mutex<AppState>>)
-> HttpResponse
{
   match check_credentials(auth, &state, DbPrivilege::CanRead)
   {
...
fn insert_person(state: web::Data<Mutex<AppState>>, query: web::Query<ToInsert>, auth: BasicAuth) -> HttpResponse
{
   match check_credentials(auth, &state, DbPrivilege::CanWrite)
   {
      Ok(_) =>
...
fn update_person(auth: BasicAuth, state: web::Data<Mutex<AppState>>, query: web::Query<ToUpdate>) -> HttpResponse
{
   match check_credentials(auth, &state, DbPrivilege::CanWrite)
   {
      Ok(_) =>
