...
impl DbConnection
{
   ...
   pub fn get_persons_by_partial_name<'a>(&'a self, subname: &'a str) -> impl Iterator<Item = &Person> + 'a
   {
   ...
   {
      if let Some((n, _)) = self.persons.iter().enumerate().find(|(_, p)| p.id == person.id)
      {
