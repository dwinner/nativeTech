...
fn get_person_name_by_id(state: web::Data<Mutex<AppState>>, info: Path<(String,)>) -> impl Responder
{
...
fn get_persons(state: web::Data<Mutex<AppState>>, query: web::Query<Filter>) -> impl Responder
{
   ...
   HttpResponse::Ok().content_type("application/json").body(
      json!(
         db_conn
            .get_persons_id_and_name_by_partial_name(&query.partial_name.clone().unwrap_or_else(|| "".to_string()),)
            .collect::<Vec<_>>()
      )
      .to_string(),
   )
