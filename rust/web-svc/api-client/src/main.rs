...
async fn request(url: Uri, method: Method, body: Option<String>)
-> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
   ...
   if res.headers().contains_key(CONTENT_TYPE)
   {
      ...
      {
         println!("{}", &str_buf.to_colored_json_auto()?);
      }
      else
      {
         println!("{}", &str_buf);
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
   ...
   match cli.command
   {
      Commands::List => request(uri_builder.path_and_query("/v1/todos").build()?, Method::GET, None).await,
      Commands::Delete { id } =>
      {
         request(
