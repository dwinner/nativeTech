fn main()
{
   ...
   println!(
      ...
      tera_engine.render("id_template", &numeric_id.clone()).unwrap()
   );
   ...
   println!(
      ...
      tera_engine.render("person_id_template", &one_person.clone()).unwrap()
   );

   tera_engine
      .add_raw_template(
         "possible_person_id_template",
         "{%if person%}Id: {{person.id}}{%else%}No person{%endif%}",
      )
      .unwrap();

   println!(
      "possible_person_id_template with one_person: [{}]",
      tera_engine.render("possible_person_id_template", &one_person).unwrap()
   );
