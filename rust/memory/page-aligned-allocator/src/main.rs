...
fn mprotect_readwrite(data: &[u8]) -> Result<(), std::io::Error>
{
   ...
   #[cfg(unix)]
   {
      ...
      let ret = unsafe { c_mprotect(data.as_ptr() as *mut c_void, data.len() - 1, PROT_READ | PROT_WRITE) };
      match ret
      {
         0 => Ok(()),
   ...
   #[cfg(windows)]
   {
      ...
      let res = unsafe { VirtualProtect(data.as_ptr() as LPVOID, data.len() - 1, PAGE_READWRITE, &mut old) };
      match res
      {
         1 => Ok(()),
         _ => Err(std::io::Error::last_os_error()),
...
fn mprotect_noaccess(data: &[u8]) -> Result<(), std::io::Error>
{
   ...
   #[cfg(windows)]
   {
      ...
      let res = unsafe { VirtualProtect(data.as_ptr() as LPVOID, data.len() - 1, PAGE_NOACCESS, &mut old) };
      match res
      {
         1 => Ok(()),
         _ => Err(std::io::Error::last_os_error()),
...
unsafe impl Allocator for PageAlignedAllocator
{
   fn allocate(&self, layout: Layout) -> Result<ptr::NonNull<[u8]>, AllocError>
   {
      ...
      let out = {
         ...
         unsafe { VirtualAlloc(ptr::null_mut(), size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE) }
      };

      // lock the pages at the fore of the region
      let fore_protected_region = unsafe { std::slice::from_raw_parts_mut(out as *mut u8, pagesize) };
      ...
      let aft_protected_region =
         unsafe { std::slice::from_raw_parts_mut(out.add(aft_protected_region_offset) as *mut u8, pagesize) };
      mprotect_noaccess(aft_protected_region)
         .map_err(|err| eprintln!("mprotect error = {:?}, in allocator", err))
         .ok();

      let slice = unsafe { std::slice::from_raw_parts_mut(out.add(pagesize) as *mut u8, layout.size()) };
   ...
   unsafe fn deallocate(&self, ptr: ptr::NonNull<u8>, layout: Layout)
   {
      ...
      let aft_protected_region_offset = pagesize + _page_round(layout.size(), pagesize);
      let aft_protected_region = std::slice::from_raw_parts_mut(ptr.add(aft_protected_region_offset), pagesize);
