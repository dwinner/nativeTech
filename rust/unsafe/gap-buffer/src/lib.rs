...
mod gap
{
   ...
   /// remove elements at any position in constant time. Indexing is also
   /// constant time. However, changing the position at which insertion and
   /// removal occur takes time proportional to the distance the insertion
   ...
   impl<T> GapBuffer<T>
   {
      ...
      fn enlarge_gap(&mut self)
      {
         ...
         // because the Vector's length is zero.
         self.storage = new;
         self.gap = new_gap;
      }
   }
