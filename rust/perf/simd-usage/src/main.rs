...
fn main()
{
   ...
   for _ in 0..100_000
   {
      let c: Vec<_> = a.iter().zip(b.iter()).map(|(l, r)| l.wrapping_mul(*r)).collect();
      let d: Vec<_> = a.iter().zip(c.iter()).map(|(l, r)| l.wrapping_add(*r)).collect();
      let e: Vec<_> = c.iter().zip(d.iter()).map(|(l, r)| l.wrapping_mul(*r)).collect();
