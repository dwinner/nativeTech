...
fn generate_random_values(count: usize) -> Vec<i64>
{
   ...
   for _ in 0..count
   {
      ret.push(rng.random_range(i64::MIN..i64::MAX));
   }
...
fn generate_random_strings(count: usize) -> Vec<String>
{
   ...
   for _ in 0..count
   {
      ...
      for _ in 0..50000
      {
         str.push(char::from_u32(rng.random_range(0..127)).unwrap());
      }
...
fn main()
{
   ...
   let start = Instant::now();
   let _matches1: Vec<_> = data.iter().filter(|str| re.is_match(str)).collect();
   let finish = Instant::now() - start;
   println!("Regex took {}s", finish.as_secs_f64());

   let start = Instant::now();
   let _matches2: Vec<_> = data.par_iter().filter(|str| re.is_match(str)).collect();
   let finish = Instant::now() - start;
