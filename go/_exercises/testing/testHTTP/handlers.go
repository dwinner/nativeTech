...
func (handler notAllowedHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
   MethodNotAllowedHandler(writer, request)
}
...
func DeleteHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   foundUser := FindUserId(intId)
   if foundUser.Username != "" {
      log.Println("About to delete:", foundUser)
...
func GetIdHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   foundUser := FindUserName(username)
   if foundUser.Id != 0 {
      writer.WriteHeader(http.StatusFound)
      err := foundUser.ToJson(writer)
      if err != nil {
         writer.WriteHeader(http.StatusBadRequest)
         log.Println(err)
         return
...
func GetUserDataHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   user := FindUserId(intId)
   if user.Id != 0 {
      err := user.ToJson(writer)
      if err != nil {
         writer.WriteHeader(http.StatusBadRequest)
         log.Println(err)
      }
...
func UpdateHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   log.Println(users)
   foundUser := FindUserName(users[1].Username)
   foundUser.Username = users[1].Username
   foundUser.Password = users[1].Password
   foundUser.Admin = users[1].Admin

   if !UpdateUser(foundUser) {
      log.Println("Update failed:", foundUser)
      writer.WriteHeader(http.StatusBadRequest)
      return
   }

   log.Println("Update successful:", foundUser)
   writer.WriteHeader(http.StatusOK)
}
...
func LoginHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   foundUser := FindUserName(user.Username)
   log.Println("Logging in:", foundUser)

   foundUser.LastLogin = time.Now().Unix()
   foundUser.Active = 1
   if UpdateUser(foundUser) {
      log.Println("User updated:", foundUser)
      writer.WriteHeader(http.StatusOK)
   } else {
      log.Println("Update failed:", foundUser)
      writer.WriteHeader(http.StatusBadRequest)
...
func LogoutHandler(writer http.ResponseWriter, request *http.Request) {
   ...
   foundUser := FindUserName(user.Username)
   log.Println("Logging out:", foundUser.Username)
   foundUser.Active = 0
   if UpdateUser(foundUser) {
      log.Println("User updated:", foundUser)
      writer.WriteHeader(http.StatusOK)
