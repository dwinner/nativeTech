...
type DataFileCollection []DataFile

// Len Implement sort.Interface
func (a DataFileCollection) Len() int {
   return len(a)
}

func (a DataFileCollection) Less(i, j int) bool {
   return a[i].Mean < a[j].Mean
}

func (a DataFileCollection) Swap(i, j int) {
   a[i], a[j] = a[j], a[i]
}

var files DataFileCollection
...
func process(file string) {
   ...
   fmt.Println("Normalized length:", len(normalized), values[10:15], normalized[10:15])
}
